...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  目付:      Sonnet（仕様管理・整合性評価）"
            echo "  検使:      Sonnet（テスト戦略・品質ゲート）"
            echo "  足軽1-5:   Sonnet（実働部隊）"
            echo ""
            echo "陣形:"
            echo "  平時の陣（デフォルト）: 足軽1-5=Sonnet, 目付=Sonnet, 検使=Sonnet, 軍師=Opus"
            echo "  決戦の陣（--kessen）:   全足軽=Opus, 目付=Sonnet, 検使=Sonnet, 軍師=Opus"
            echo ""
            echo "表示モード:"
...
show_battle_cry() {
    ...
    echo -e "\033[1;34m  ║\033[0m          \033[1;37m【 足軽隊列・五名 + 目付 + 検使 + 軍師 配備 】\033[0m              \033[1;34m║\033[0m"
    echo -e "\033[1;34m  ╚═════════════════════════════════════════════════════════════════════════════╝\033[0m"
...
ASHIGARU_EOF
    ...
    echo -e "\033[1;33m  ┃\033[0m  \033[1;35m将軍\033[0m: 統括 \033[1;31m家老\033[0m: 管理 \033[1;33m軍師\033[0m: 戦略 \033[1;36m目付\033[0m: 仕様 \033[1;31m検使\033[0m: 検分 \033[1;34m足軽\033[0m:×5 \033[1;33m┃\033[0m"
    echo -e "\033[1;33m  ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\033[0m"
    echo ""
}
...
EOF
    ...
    # 検使タスクファイルリセット
    cat > ./queue/tasks/kenshi.yaml << EOF
# 検使専用タスクファイル
task:
  task_id: null
  parent_cmd: null
...
EOF
    ...
    # 検使レポートファイルリセット
    cat > ./queue/reports/kenshi_report.yaml << EOF
worker_id: kenshi
task_id: null
timestamp: ""
status: idle
...
EOF
    ...
    # agent inbox リセット
    for agent in shogun karo $_ASHIGARU_IDS_STR metsuke kenshi gunshi; do
...
" 2>/dev/null
    ...
    # 目付（pane _ASHIGARU_COUNT+1）: 仕様管理・整合性チェック専任
    p=$((PANE_BASE + _ASHIGARU_COUNT + 1))
    _metsuke_cli_type="claude"
    _metsuke_cmd="claude --model sonnet --dangerously-skip-permissions"
    if [ "$CLI_ADAPTER_LOADED" = true ]; then
        _metsuke_cli_type=$(get_cli_type "metsuke")
        _metsuke_cmd=$(build_cli_command "metsuke")
    fi
    _startup_prompt=$(get_startup_prompt "metsuke" 2>/dev/null)
    if [[ -n "$_startup_prompt" ]]; then
        _metsuke_cmd="$_metsuke_cmd \"$_startup_prompt\""
    fi
    tmux set-option -p -t "multiagent:agents.${p}" @agent_cli "$_metsuke_cli_type"
    tmux send-keys -t "multiagent:agents.${p}" "$_metsuke_cmd"
    tmux send-keys -t "multiagent:agents.${p}" Enter
    _metsuke_display=$(get_model_display_name "metsuke" 2>/dev/null || echo "Sonnet")
    tmux set-option -p -t "multiagent:agents.${p}" @model_name "$_metsuke_display" 2>/dev/null || true
    log_info "  └─ 目付（${_metsuke_display}）、召喚完了"

    # 検使（pane _ASHIGARU_COUNT+2）: テスト実行・品質門判定専任
    p=$((PANE_BASE + _ASHIGARU_COUNT + 2))
    _kenshi_cli_type="claude"
    _kenshi_cmd="claude --model sonnet --dangerously-skip-permissions"
    if [ "$CLI_ADAPTER_LOADED" = true ]; then
        _kenshi_cli_type=$(get_cli_type "kenshi")
        _kenshi_cmd=$(build_cli_command "kenshi")
    fi
    _startup_prompt=$(get_startup_prompt "kenshi" 2>/dev/null)
    if [[ -n "$_startup_prompt" ]]; then
        _kenshi_cmd="$_kenshi_cmd \"$_startup_prompt\""
    fi
    tmux set-option -p -t "multiagent:agents.${p}" @agent_cli "$_kenshi_cli_type"
    tmux send-keys -t "multiagent:agents.${p}" "$_kenshi_cmd"
    tmux send-keys -t "multiagent:agents.${p}" Enter
    _kenshi_display=$(get_model_display_name "kenshi" 2>/dev/null || echo "Sonnet")
    tmux set-option -p -t "multiagent:agents.${p}" @model_name "$_kenshi_display" 2>/dev/null || true
    log_info "  └─ 検使（${_kenshi_display}）、召喚完了"

    # 軍師（pane _ASHIGARU_COUNT+3）: Opus Thinking — 戦略立案・設計判断専任
    p=$((PANE_BASE + _ASHIGARU_COUNT + 3))
    ...
    if [ "$KESSEN_MODE" = true ]; then
        ...
        log_success "✅ 平時の陣で出陣（家老=Sonnet, 足軽=Sonnet, 目付=Sonnet, 検使=Sonnet, 軍師=Opus）"
    fi
    echo ""
...
NINJA_EOF
    ...
    nohup bash "$SCRIPT_DIR/scripts/inbox_watcher.sh" "metsuke" "multiagent:agents.${p}" "$_metsuke_watcher_cli" \
        >> "$SCRIPT_DIR/logs/inbox_watcher_metsuke.log" 2>&1 &
    disown

    # 検使のwatcher
    p=$((PANE_BASE + _ASHIGARU_COUNT + 2))
    _kenshi_watcher_cli=$(tmux show-options -p -t "multiagent:agents.${p}" -v @agent_cli 2>/dev/null || echo "claude")
    nohup bash "$SCRIPT_DIR/scripts/inbox_watcher.sh" "kenshi" "multiagent:agents.${p}" "$_kenshi_watcher_cli" \
        >> "$SCRIPT_DIR/logs/inbox_watcher_kenshi.log" 2>&1 &
    disown
    ...
    log_success "  └─ $((_ASHIGARU_COUNT + 5))エージェント分のinbox_watcher起動完了（将軍+家老+足軽${_ASHIGARU_COUNT}+目付+検使+軍師）"
...
echo "     └─────────────────────────────┘"
echo ""
echo "     【multiagentセッション】家老・足軽・目付・検使・軍師の陣（3x3 = 9ペイン）"
echo "     ┌─────────┬─────────┬─────────┐"
echo "     │  karo   │ashigaru3│metsuke  │"
echo "     │  (家老) │ (足軽3) │ (目付)  │"
echo "     ├─────────┼─────────┼─────────┤"
echo "     │ashigaru1│ashigaru4│ kenshi  │"
echo "     │ (足軽1) │ (足軽4) │ (検使)  │"
echo "     ├─────────┼─────────┼─────────┤"
