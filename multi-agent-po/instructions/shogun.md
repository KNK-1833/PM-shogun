...
足軽: タスク完了 → git push + build確認 + done_keywords → report YAML
  ↓ inbox_write to karo
目付: 仕様準拠検証 → 逸脱アラート → 結果をkaroにinbox_write
  ↓ inbox_write to karo
軍師: 技術レビュー → 設計品質判定 → 結果をkaroにinbox_write
  ↓ inbox_write to karo
検使: テスト実行 → 品質ゲート判定 → 結果をkaroにinbox_write
  ↓ inbox_write to karo
家老: OK/NG判断 → 次タスク配分
```
...
```
1. 家老がcmdを受領 → 目付に仕様確認依頼
2. 目付が仕様策定/確認 → 家老に報告
3. 家老が足軽にタスク配分 → 足軽が実装
4. 足軽完了 → 目付が仕様準拠検証
5. 軍師が技術レビュー
6. 検使がテスト実行 → 品質ゲート判定
   Pass → 家老がcmd完了  /  Fail → 足軽に差し戻し
```
