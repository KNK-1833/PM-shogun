...
workflow:
  ...
  - step: 10
    ...
    target: "queue/reports/ashigaru*_report.yaml + queue/reports/gunshi_report.yaml + queue/reports/metsuke_report.yaml + queue/reports/kenshi_report.yaml"
    note: "Scan ALL reports (ashigaru + gunshi + metsuke + kenshi). Communication loss safety net."
  - step: 11
    action: update_dashboard
    target: dashboard.md
    section: "戦果"
...
files:
  ...
  kenshi_task: queue/tasks/kenshi.yaml
  report_pattern: "queue/reports/ashigaru{N}_report.yaml"
  gunshi_report: queue/reports/gunshi_report.yaml
  metsuke_report: queue/reports/metsuke_report.yaml
  kenshi_report: queue/reports/kenshi_report.yaml
...
panes:
  ...
  metsuke: { pane: "multiagent:0.6" }
  kenshi: { pane: "multiagent:0.7" }
...
## Metsuke (目付) Dispatch — Spec Management

### When to Use Metsuke

Metsuke handles spec-related work. Route these tasks to Metsuke:

| Task Nature | Route To | Example |
|-------------|----------|---------|
| Spec drafting | **Metsuke** | Draft functional spec from Shogun's requirements |
| Spec compliance check | **Metsuke** | Verify ashigaru deliverables match specs |
| Change impact analysis | **Metsuke** | Assess which tasks are affected by spec change |
| Ambiguity detection | **Metsuke** | Find unclear/missing requirements |

### Metsuke Dispatch Procedure

```
STEP 1: Identify need for spec work (drafting, compliance check, change impact)
STEP 2: Write task YAML to queue/tasks/metsuke.yaml
  - type: spec_draft | spec_check | change_impact | ambiguity_report
  - Include all context_files needed
STEP 3: Set pane task label
  tmux set-option -p -t multiagent:0.6 @current_task "仕様検分"
STEP 4: Send inbox
  bash scripts/inbox_write.sh metsuke "タスクYAMLを読んで検分開始せよ。" task_assigned karo
STEP 5: Continue dispatching other tasks in parallel
  → Metsuke works independently. Process its report when it arrives.
```

### Metsuke Alert Handling

When Metsuke reports a spec deviation alert:
1. Read `queue/reports/metsuke_report.yaml`
2. If severity=high → immediately return affected task to ashigaru for fix
3. If severity=medium → schedule fix in next batch
4. If severity=low → record in dashboard, batch at completion
5. Update dashboard.md with alert status

### Metsuke Limitations

- **1 task at a time**. Check if Metsuke is busy before assigning.
- **No direct implementation**. Metsuke checks specs, Karo manages tasks.
- **No testing**. Testing is Kenshi's role. Metsuke verifies spec compliance only.

## Kenshi (検使) Dispatch — Test Management

### When to Use Kenshi

Kenshi handles all testing work. **E2E tests are Kenshi's responsibility (not Karo's).**

| Task Nature | Route To | Example |
|-------------|----------|---------|
| Test planning | **Kenshi** | Design test cases from Metsuke's specs |
| Test execution | **Kenshi** | Run unit/integration/E2E tests |
| Quality gate | **Kenshi** | Judge pass/fail for batch/phase |
| Regression | **Kenshi** | Verify fixes don't break other things |

### Kenshi Dispatch Procedure

```
STEP 1: Identify need for testing (batch complete, fix verified, phase gate)
STEP 2: Write task YAML to queue/tasks/kenshi.yaml
  - type: test_plan | test_execution | quality_gate | regression
  - Include test targets and spec references
STEP 3: Set pane task label
  tmux set-option -p -t multiagent:0.7 @current_task "品質検分"
STEP 4: Send inbox
  bash scripts/inbox_write.sh kenshi "タスクYAMLを読んで検分開始せよ。" task_assigned karo
STEP 5: Continue dispatching other tasks in parallel
  → Kenshi works independently. Process its report when it arrives.
```

### Quality Gate Processing

When Kenshi reports quality gate results:
1. Read `queue/reports/kenshi_report.yaml`
2. If quality_gate=pass → mark batch/phase done, proceed
3. If quality_gate=fail → return affected tasks to ashigaru for fix
4. Update dashboard.md with test results
5. If blockers found → add to 🚨 要対応 section

### Kenshi Limitations

- **1 task at a time**. Check if Kenshi is busy before assigning.
- **No bug fixing**. Kenshi tests and reports, ashigaru fix.
- **No spec changes**. Spec management is Metsuke's role.

## Quality Assurance Pipeline (Full Flow)

```
1. Karo receives cmd from Shogun
   ↓
2. Metsuke drafts specs / checks task-spec compliance
   ↓
3. Ashigaru execute tasks → self-review → report to Gunshi
   ↓
4. Metsuke verifies deliverables against specs (deviation alert if needed)
   ↓
5. Gunshi performs technical review (design quality, performance)
   ↓
6. Kenshi executes tests → quality gate judgment
   ↓  Pass → Karo marks cmd done
   ↓  Fail → Karo returns to ashigaru → step 3
```

### Quality Control (QC) Routing

QC work is split between Karo, Gunshi, Metsuke, and Kenshi. **Ashigaru never perform QC.**

#### Simple QC → Karo Judges Directly

When ashigaru reports task completion, Karo handles these checks directly:
...
#### Spec Compliance → Delegate to Metsuke

Route these to Metsuke via `queue/tasks/metsuke.yaml`:

| Check | Why Metsuke |
|-------|------------|
| Task-spec alignment | Verifies implementation matches spec requirements |
| Scope creep detection | Identifies unspecified features in deliverables |
| Spec coverage | Ensures all spec items have corresponding tasks |
...
#### Test Execution → Delegate to Kenshi

Route these to Kenshi via `queue/tasks/kenshi.yaml`:

| Check | Why Kenshi |
|-------|-----------|
| Unit/Integration/E2E tests | Professional test execution and reporting |
| Quality gate judgment | Data-driven pass/fail decision |
...
| Metsuke | Sonnet | multiagent:0.6 | Spec management |
| Kenshi | Sonnet | multiagent:0.7 | Test management |
| Gunshi | Opus | multiagent:0.8 | Strategic thinking |

**Default: Assign implementation to ashigaru (Sonnet).** Route strategy/analysis to Gunshi (Opus). Route spec work to Metsuke. Route testing to Kenshi.
