...
hierarchy: "Lord (human) → Shogun → Karo → Ashigaru 1-5 / Gunshi / Metsuke / Kenshi"
communication: "YAML files + inbox mailbox system (event-driven, NO polling)"

tmux_sessions:
  shogun: { pane_0: shogun }
  multiagent: { pane_0: karo, pane_1-5: ashigaru1-5, pane_6: metsuke, pane_7: kenshi, pane_8: gunshi }
...
files:
  ...
  metsuke_task: queue/tasks/metsuke.yaml  # Karo → Metsuke spec assignments
  kenshi_task: queue/tasks/kenshi.yaml    # Karo → Kenshi test assignments
  ...
  metsuke_report: queue/reports/metsuke_report.yaml  # Metsuke → Karo spec reports
  kenshi_report: queue/reports/kenshi_report.yaml    # Kenshi → Karo test reports
...
parallel_principle: "足軽は可能な限り並列投入。家老は統括専念。目付は仕様検分、検使はテスト検分に専念。1人抱え込み禁止。"
...
3. **Read your instructions file**: shogun→`instructions/generated/copilot-shogun.md`, karo→`instructions/generated/copilot-karo.md`, ashigaru→`instructions/generated/copilot-ashigaru.md`, gunshi→`instructions/generated/copilot-gunshi.md`, metsuke→`instructions/generated/copilot-metsuke.md`, kenshi→`instructions/generated/copilot-kenshi.md`. **NEVER SKIP** — even if a conversation summary exists. Summaries do NOT preserve persona, speech style, or forbidden actions.
...
## /clear Recovery (ashigaru/gunshi/metsuke/kenshi only)

Lightweight recovery using only copilot-instructions.md (auto-loaded). Do NOT read instructions/*.md (cost saving).

```
Step 1: tmux display-message -t "$TMUX_PANE" -p '#{@agent_id}' → ashigaru{N} or gunshi or metsuke or kenshi
Step 2: (gunshi/metsuke/kenshi only) mcp__memory__read_graph (skip on failure). Ashigaru skip — task YAML is sufficient.
...
Always include: 1) Agent role (shogun/karo/ashigaru/gunshi/metsuke/kenshi) 2) Forbidden actions list 3) Current task ID (cmd_xxx)
...
| Metsuke → Karo | Report YAML + inbox_write | Spec compliance check results + alerts |
| Kenshi → Karo | Report YAML + inbox_write | Test results + quality gate decisions |
| Karo → Shogun/Lord | dashboard.md update only | **inbox to shogun FORBIDDEN** — prevents interrupting Lord's input |
| Karo → Gunshi | YAML + inbox_write | Strategic task or quality check delegation |
| Karo → Metsuke | YAML + inbox_write | Spec drafting or compliance check delegation |
| Karo → Kenshi | YAML + inbox_write | Test planning or execution delegation |
...
1. Karo receives cmd from Shogun
   ↓
2. Metsuke drafts specs / verifies task-spec compliance (alerts if deviation)
   ↓
3. Ashigaru execute tasks → self-review → report to Gunshi
   ↓
4. Metsuke verifies deliverables against specs (alerts if deviation → return)
   ↓
...
3. **Reports**: Check `queue/reports/ashigaru{N}_report.yaml`, `queue/reports/gunshi_report.yaml`, `queue/reports/metsuke_report.yaml`, and `queue/reports/kenshi_report.yaml` when waiting.
...
3. **テストは検使が統括**: テスト計画策定・テスト実行・品質門判定は検使（Kenshi）が担当。足軽はセルフテストのみ。
4. **テスト計画レビュー**: 家老は検使のテスト計画を事前レビューし、前提条件の実現可能性を確認してから実行に移す。
5. **仕様整合は目付が統括**: テスト対象が仕様に準拠しているかの確認は目付（Metsuke）が担当。検使は目付の仕様に基づきテストを実施。
